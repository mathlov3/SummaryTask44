            ...
            <div class="table-responsive">
                <table class="table table-list-search">
                    ...
                    <tr>
                        ...
                        <th><fmt:message key="editproduct.edit"/> </th>
                    </tr>
                    </thead>
                    <c:forEach items="${requestScope.products}" var="item">
                    <tbody>
                    <tr>
                        <td><a href="product?id=${item.id}">${item.id}</a></td>
                        <td>${item.name}</td>
                        ...
                        <c:forEach items="${applicationScope.categories}" var="c">
                            <c:if test="${item.categoryId == c.id}">
                                ${c.name}
                            </c:if>
                        </c:forEach>
                        </td>
                ...
                <ul class="pagination">
                    ...
                    <c:set var="t" value="${(sessionScope.countProducts+1)/10}"/> <%-- total amount of pages --%>
